...
class SerializerROS():
    def __init__(self):
        ...
        if self.publish_sensors:
            ...
            try:      
                ...
                pass
            try:
                for sensor, params in self.maxez1_sensors.iteritems():
                    rospy.loginfo(sensor + " " + str(params))
            except:
                pass
        ...
        while not rospy.is_shutdown():
            if self.publish_sensors:
                ...
                for sensor, params in self.maxez1_sensors.iteritems():
                    self.sensors[sensor] = self.mySerializer.get_MaxEZ1(params['trigger_pin'], params['output_pin'])
    ...
    def PingHandler(self, req):
        try:
            ...
            rospy.logerror("Bad sonar value on Ping " + str(req.pin) + ". Value was: " + str(sonar))
        return PingResponse(sonar)
    
    def MaxEZ1Handler(self, req):
        try:
            sonar = self.mySerializer.get_MaxEZ1(req.trigger_pin, req.output_pin, req.cached)
        except:
            rospy.logerror("Bad sonar value on MaxEZ1 " + str(req.trigger_pin) + ". Value was: " + str(sonar))
        return MaxEZ1Response(sonar)
