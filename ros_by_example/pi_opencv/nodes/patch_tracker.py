...
""" patch_tracker.py - Version 1.0 2011-04-28

    Copyright @ 2011 by Patrick Goebel
...
class PatchTracker(ROS2OpenCV):
    def __init__(self, node_name):
        ...
        """ Should we display a time-lapse of all markers? """
    ...
    def process_image(self, cv_image):
        ...
        #cv.ShowImage("Grey", self.depth_image)
...
#            """ Star Features """
#            star_params = (32,1,1,1,10)
#            cv.SetImageROI(self.grey, (x, y, w, h))
#            keypoints = cv.GetStarKeypoints(self.grey, cv.CreateMemStorage(0), star_params)
#            self.features = []
...
#            self.features = cv.FindCornerSubPix(self.grey, self.features,
...
#                (cv.CV_TERMCRIT_ITER | cv.CV_TERMCRIT_EPS, 20, 0.03))
    ...
    def add_features(self, cv_image, min_distance):
        ...
        #rospy.loginfo("NEW Features: " + str(i))
        
        """ Remove duplicate features """
        self.features = list(set(self.features))
        #rospy.loginfo("TOTAL Features: " + str(len(self.features)))
            ...
            for point in features_z:
                ...
                try:
                    if abs(z - mean_z) / mean_z > 0.5:
                        features_xy.remove(point)
                        #rospy.loginfo("Bad distance at: " + str(z))
                except:
                    pass
        else:
            mse_z = -1
...
#        if len(self.features) != len(features_xy):
#            """ Recompute the COG """
#            for point in self.features:
#                sum_x = sum_x + point[0]
#                sum_y = sum_y + point[1]
...
#            mean_y = sum_y / n
#            
#            """ Re-compute the MSE of the cluster """
#            sse = 0
#            for point in self.features:
#                sse = sse + (point[0] - mean_x) * (point[0] - mean_x) + (point[1] - mean_y) * (point[1] - mean_y)
#    
...
#            return ((0, 0), 0, -1)
    ...
    def detect_face(self, cv_image):
        #rospy.loginfo("Looking for face...")
    ...
    def track_camshift(self, cv_image):
        if not self.tracking:
            ...
            sel = cv.GetSubRect(self.hue, self.track_box )
        ...
        if self.track_box and self.is_rect_nonzero(self.track_box):
            ...
            (iters, (area, value, rect), track_box) = cv.CamShift(backproject, self.track_box, crit)
            self.track_box = rect
...
def main(args):
    ...
    """ Fire up the Patch Tracker node """
    PT = PatchTracker("patch_tracker")
