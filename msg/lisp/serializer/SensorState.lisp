...
(defmethod md5sum ((type (eql '<SensorState>)))
  ...
  "c775d5ae64f1f355fcb3c88b89468dd0")
(defmethod message-definition ((type (eql '<SensorState>)))
  "Returns full string definition for message of type '<SensorState>"
  (format nil "Header header~%~%string[] name~%float32[] value~%~%================================================================================~%MSG: roslib/Header~%# Standard metadata for higher-level stamped data types.~%# This is generally used to communicate timestamped data ~%# in a particular coordinate frame.~%# ~%# sequence ID: consecutively increasing ID ~%uint32 seq~%#Two-integer timestamp that is expressed as:~%# * stamp.secs: seconds (stamp_secs) since epoch~%# * stamp.nsecs: nanoseconds since stamp_secs~%# time-handling sugar is provided by the client library~%time stamp~%#Frame this data is associated with~%# 0: no frame~%# 1: global frame~%string frame_id~%~%~%"))
(defmethod serialization-length ((msg <SensorState>))
  (+ 0
     ...
     4 (reduce #'+ (slot-value msg 'value) :key #'(lambda (ele) (declare (ignorable ele)) (+ 4)))
))
