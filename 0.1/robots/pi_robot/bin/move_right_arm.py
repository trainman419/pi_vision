...
class move_arm():
    def __init__(self):
        ...
        rospy.wait_for_service('right_arm_kinematics/get_ik')
        rospy.wait_for_service('right_arm_kinematics/get_ik_solver_info')
        
        get_ik_proxy = rospy.ServiceProxy('right_arm_kinematics/get_ik', GetPositionIK, persistent=True)
        get_ik_solver_info_proxy = rospy.ServiceProxy('right_arm_kinematics/get_ik_solver_info', GetKinematicSolverInfo)

        right_arm_solver_info = get_ik_solver_info_proxy()
                    
        request = GetPositionIKRequest()
        request.timeout = rospy.Duration(5.0)
        request.ik_request.pose_stamped.header.frame_id = "right_shoulder_link";
        request.ik_request.ik_link_name = "right_shoulder_lift_link";
...
#        request.ik_request.pose_stamped.pose.orientation.w = 1
        ...
        q = quaternion_from_euler(0, 1.0, 0)
        ...
        request.ik_request.ik_seed_state.joint_state.name = right_arm_solver_info.kinematic_solver_info.joint_names
        request.ik_request.ik_seed_state.joint_state.position = [0]*len(request.ik_request.ik_seed_state.joint_state.name )
        
        arm_joints = right_arm_solver_info.kinematic_solver_info.joint_names
        ...
        arm_cmd.header.frame_id = 'right_shoulder_link'
...
if __name__ == '__main__':
    try:
        ...
        rospy.loginfo("Shutting down right arm forward kinematics node node...")
