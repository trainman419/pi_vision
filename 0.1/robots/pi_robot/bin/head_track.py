...
import roslib; roslib.load_manifest('head_tracking_tutorial')
import rospy
...
class head_track():
    def __init__(self):
        ...
        self.min_tilt = rospy.get_param("~min_tilt", radians(-60))
        
        r = rospy.Rate(self.rate) 
        
        self.head_cmd = JointState()
        self.joints = ["head_pan_joint", "head_tilt_joint"]
        #self.joints = ["left_wrist_joint", "right_wrist_joint"]
        ...
        rospy.Subscriber('/joint_states', JointState, self.getJointStates)

        while not rospy.is_shutdown():
            self.head_cmd.position[0] = random.sample((self.min_pan, self.max_pan), 1)[0]
            self.head_cmd.position[1] = random.sample((self.min_tilt, self.max_tilt), 1)[0]
            
            self.head_cmd.velocity[0] = random.randrange(0, 2)
            self.head_cmd.velocity[1] = random.randrange(0, 2)
...
#                self.last_tracking_seq = self.tracking_seq
    ...
    def getJointStates(self, msg):
        if abs(msg.velocity[0]) > 2 or abs(msg.velocity[1]) > 2:
            rospy.loginfo("MAX SPEED EXCEEDED! " + str(msg.velocity))
