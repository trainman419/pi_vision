...
#                """ Optionally re-run the Haar detector on a region surrounding the current features """
#                if self.track_box and self.is_rect_nonzero(self.track_box):
#                    face_box = self.redetect_face(cv_image)
#                    if face_box:
#                        rospy.loginfo(face_box)
#                        #os._exit(1)
#                        self.detect_box = face_box
#                        self.track_box = None
    ...
    def redetect_face(self, cv_image):
        """ Look only in the region of the current tracked features """        
...
#        roi_y = int(y - h / 2)
        ...
        cv.Rectangle(self.marker_image, (roi_x, roi_y), (roi_x + roi_w, roi_y + roi_h), cv.RGB(255, 255, 0), 2, 8, 0)
        ...
        """ First check one of the frontal templates """
        if self.cascade_frontal_alt:
            faces = cv.HaarDetectObjects(grey, self.cascade_frontal_alt, cv.CreateMemStorage(0),
                                          self.haar_scale, self.min_neighbors, self.haar_flags, self.min_size)
                                         
#        """ If that fails, check the profile template """
#        if not faces:
#            if self.cascade_profile:
#                faces = cv.HaarDetectObjects(grey, self.cascade_profile, cv.CreateMemStorage(0),
#                                             self.haar_scale, self.min_neighbors, self.haar_flags, self.min_size)
#
#            if not faces:
#                """ If that fails, check a different frontal profile """
#                if self.cascade_frontal_alt2:
#                    faces = cv.HaarDetectObjects(small_image, self.cascade_frontal_alt2, cv.CreateMemStorage(0),
#                                         self.haar_scale, self.min_neighbors, self.haar_flags, self.min_size)
        
        if not faces:
            if self.show_text:
                text_font = cv.InitFont(cv.CV_FONT_VECTOR0, 3, 2, 0, 3)
                cv.PutText(self.marker_image, "CAN'T DETECT FACE", (50, int(self.image_size[1] * 0.9)), text_font, cv.RGB(255, 255, 0))
            return None
...
#                    continue
            ...
            face_box = (pt1[0] + roi_x, pt1[1] + roi_y, face_width, face_height)
