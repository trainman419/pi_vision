...
""" camshift.py - Version 1.0 2011-04-28
...
class CamShiftNode(ROS2OpenCV):
    ...
    def filter_image(self, cv_image):
        if not self.backproject:
            ...
            self.hue2 = cv.CreateImage(self.image_size, 8, 1)
            self.mask = cv.CreateImage(self.image_size, 8, 1)
            self.backproject_mat = cv.CreateMat(self.image_size[0], self.image_size[1])
            
        #cv.Copy(cv_image, self.track)
        ...
        #self.hue2 = cv.GetImage(self.hue)
        ...
        if self.track_window and self.is_rect_nonzero(self.track_window):
            ...
            """ """
            rospy.loginfo(self.track_window)
            """ """
            (iters, (area, value, rect), track_box) = cv.CamShift(self.backproject, self.track_window, crit)
#            x, y, w, h = rect
#            cv.Rectangle(cv_image, (x, y), (x + w, y + h), (0, 0, 255), 2)
            self.track_window = rect       
     
        """ If mouse is pressed, recompute the histogram """
        if self.drag_start and self.is_rect_nonzero(self.selection):
            cv.SetImageROI(self.hue, self.selection)
            cv.SetImageROI(self.mask, self.selection)
            
            cv.ShowImage("Mask", self.mask)
            
            cv.CalcArrHist([self.hue], self.hist, 0, self.mask)
            ...
            self.ROI.x_offset = int(min(self.image_size[0], max(0, track_box[0][0] - track_box[1][0] / 2)))
            self.ROI.y_offset = int(min(self.image_size[1], max(0, track_box[0][1] - track_box[1][1] / 2)))
            self.ROI.width = int(track_box[1][0])
            self.ROI.height = int(track_box[1][1])
                    
            self.pubROI.publish(self.ROI)

        #cv.ShowImage("Track", self.track)
